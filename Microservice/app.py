...
def create_email_and_send(customer_id, order_status, order_number):
    ...
    UserQry = "select * from `users`.customers where id = '{0}'".format(
        int(customer_id)
    )
    cur = conn.cursor()
    cur.execute(UserQry)
...
def send_mail(subject, body, email_receiver):
    email_sender = (
        "suhasmkumar66@gmail.com"  # personal google account is set to send email
    )
...
class RegisterClass:
    def on_post(self, req, resp):
        ...
        if (
            ...
            sQry = "select * from `users`.customers where username = '{0}'".format(
                data["username"]
            )
            cur = conn.cursor()
            cur.execute(sQry)
            ...
            if output is not None:
                ...
                return
            sQry = "select * from `users`.customers where email = '{0}'".format(
                data["email"]
            )
            cur.execute(sQry)
            output = cur.fetchone()
...
class GetProductsClass:
    def on_post(self, req, resp):
        ...
        if "category_id" in data:
            sQry = "select * from `products`.product_list where category_id = '{0}'".format(
...
class PlaceOrderClass:
    def on_post(self, req, resp):
        ...
        if "order_number" in data and "delivery_type" in data and "customer_id" in data:
            sQry = (
                "select * from `orders`.cart_details where order_number = '{0}'".format(
                    data["order_number"]
                )
            )
            cur = conn.cursor()
            ...
            for row in result:
                ...
                sPQry = "select * from `products`.product_list where product_id = '{0}'".format(
                    row["product_id"]
                )
                cur.execute(sPQry)
                output = cur.fetchone()
                if output is not None:
                    ...
                    UptdQry = "Update `products`.product_list set quantity = '{0}' where product_id = '{1}'".format(
                        int(updated_quantity), row["product_id"]
                    )
                    cur.execute(UptdQry)
                    conn.commit()

            dQry = (
                "delete from `orders`.cart_details where order_number = '{0}'".format(
                    data["order_number"]
                )
            )
            cur = conn.cursor()
    ...
    def on_post(self, req, resp):
        ...
        if "order_number" in data and "order_status" in data and "customer_id" in data:
            ...
            if data["order_status"] == "Cancelled":
                sQry = "select * from `orders`.order_details where order_number = '{0}'".format(
                    data["order_number"]
                )
                cur = conn.cursor()
                cur.execute(sQry)
            ...
            Uqry = "Update `orders`.order_details set order_status = '{0}',datetime = '{1}' where order_number = '{2}'".format(
                data["order_status"],
                now.strftime("%Y-%m-%d %H:%M:%S"),
                data["order_number"],
            )
            ...
            result = {
                "msg": "order status updated sucessfully",
                "OrderNo": data["order_number"],
